...
/**
 * <p>
 * This class represents the file containing 10 millions of ratings from the
 * MovieLens data set.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * If a rating is on a movie with a MovieLens id that do not have a
 * corresponding IMDb id, then this rating is ignored.
 * </p>
 */
public class TenMillionRatingFile implements ImportableRatingFile {
	/**
	 * This iterator converts each given line from the 10 million rating file
	 * into a Mynemo rating.
	 */
	private class ConverterIterator extends UnmodifiableIterator<MynemoRating> {

		private final Iterator<String> delegate;

		public ConverterIterator(Iterator<String> filteredIterator) {
			delegate = filteredIterator;
		}
		...
		public MynemoRating next() {
			...
			try {
				nextRating = MynemoRating.createRatingFromMovieLens(user,
						imdbId, value);
	...
	/**
	 * This predicate is true only if the given line from the 10 million rating
	 * file is a rating on a movie that has an IMDb id.
	 */
	...
	private static final Object KNOWN_FIRST_LINE = "1::122::5::838985046";
	private static final int MOVIE_INDEX = 1;
	private static final String MOVIELENS_VALUE_SEPARATOR = "::";
	private static final Pattern PATTERN = Pattern
			.compile(MOVIELENS_VALUE_SEPARATOR);
	private static final int USER_INDEX = 0;
	private static final int VALUE_INDEX = 2;

	/**
	 * Returns <code>true</code> if the given file can be parsed,
	...
	public TenMillionRatingFile(String ratingFilepath, String movieFilepath)
			throws IOException {
		checkArgument(ratingFilepath != null,
				"The rating filepath must be not null.");
		checkArgument(movieFilepath != null,
				"The movie filepath must be not null.");

		idConverter = new IdConverter(movieFilepath);
		lineIterator = new LineIterator(new BufferedReader(new FileReader(
				ratingFilepath)));
	}
	...
	public Iterator<MynemoRating> iterator() {
		return new ConverterIterator(Iterators.filter(lineIterator,
				new HasCorrespondingImdbId()));
	}
}
