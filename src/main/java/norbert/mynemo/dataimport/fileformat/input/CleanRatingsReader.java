...
/**
 * This class discards the first two lines and the last line from a stream. The
 * last line must begin with the character '<code>&lt;</code>'. The first line
...
public class CleanRatingsReader extends Reader {
	...
	public int read(char[] buffer, int offset, int length) throws IOException {
		...
		if (firstReading) {
			...
			checkState(firstLine.equals(EXPECTED_FIRST_LINE),
					"Error: the first line of the file must be \""
							+ EXPECTED_FIRST_LINE + "\".");
			reader.readLine();
			firstReading = false;
		}
