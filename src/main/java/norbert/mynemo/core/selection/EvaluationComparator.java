...
/**
 * <p>
 * This class provides a comparison between recommender evaluations. An
 * evaluation A is lesser than another evaluation B if its metric value is
 * lesser than the metric value of B. Evaluations with a coverage less or equal
 * to a given number are greater than those with an acceptable coverage. Two
...
public class EvaluationComparator implements Comparator<RecommenderEvaluation> {
	...
	public int compare(RecommenderEvaluation o1, RecommenderEvaluation o2) {
		...
		if (Double.isNaN(o1Value)
				|| o1.getEvaluationReport().getCoverage() <= minimumCoverage) {
			o1Value = Double.MAX_VALUE;
		}
		if (Double.isNaN(o2Value)
				|| o2.getEvaluationReport().getCoverage() <= minimumCoverage) {
