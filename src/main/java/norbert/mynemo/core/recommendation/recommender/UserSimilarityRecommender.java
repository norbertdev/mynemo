...
/**
 * This class represents several recommender builders. The recommenders are
 * based on similarities between the users. A builder is chosen via the
 * {@link #UserSimilarityRecommender(UserBasedRecommenderConfiguration)
 * constructor}.
 */
public class UserSimilarityRecommender implements RecommenderBuilder {
	...
	/**
	 * <p>
	 * Creates a recommender builder specialized in the user-similarity based
	 * algorithms.
	 * </p>
	 * <p>
	 * Whereas the similarity matrix can be reuse, the neighborhood is never
	 * reused.
	 * </p>
	 */
	public UserSimilarityRecommender(
	...
	public Recommender buildRecommender(DataModel dataModel)
		...
		if (!configuration.allowCachedSimilarityReuse()) {
			...
			if (cachedSimilarity == null) {
				// lazy initialization
				cachedSimilarity = createSimilarity(configuration
						.getDataModel());
			}
		...
		UserNeighborhood neighborhood = new NearestNUserNeighborhood(
				configuration.getNeighborNumber(), currentSimilarity, dataModel);
	...
	private UserSimilarity createSimilarity(DataModel dataModel)
			throws TasteException {
		...
		case USER_SIMILARITY_WITH_EUCLIDEAN_DISTANCE:
			selectedSimilarity = new EuclideanDistanceSimilarity(dataModel,
		...
		case USER_SIMILARITY_WITH_PEARSON_CORRELATION:
			selectedSimilarity = new PearsonCorrelationSimilarity(dataModel,
					Weighting.UNWEIGHTED);
			break;
		...
		int cacheSize = Math.min(
				dataModel.getNumUsers() * dataModel.getNumUsers(),
				MAXIMUM_CACHE_SIZE);
