...
/**
 * This class enables to parse a command line to select the best suited
...
public class SelectCommandParser {
	...
	private static final String ALGORITHMS_DESCRIPTION = "list of"
			+ " space-separated algorithms to be used by the selection"
			+ " process. If no list is provided, the selection is done"
			+ " among some fast algorithms.";
	private static final String ALGORITHMS_LONG_OPTION = "algorithms";

	private static final String COMMAND_SYNTAX = "select  --data-model <file>"
			+ "  --user <id>  [--algorithms <algo1> [<algo2>…]]"
			+ "  [--metric <metric>]  [--speed <speed>]"
			+ "  [--coverage <number>]";

	// coverage
	private static final String COVERAGE_ARG_NAME = "number";
	private static final char COVERAGE_CHAR_OPTION = 'c';
	private static final String COVERAGE_DESCRIPTION = "minimum coverage"
			+ " required for the evaluations. A recommender may be unable to"
			+ " provide a recommendation for an item. The coverage reflects"
			+ " the ratio between the number of provided recommendation for"
			+ " the already rated items and the number of already rated item."
			+ " The coverage must be between 0 and 1. The default value is "
	...
	private static final String METRIC_DESCRIPTION = "metric to evaluate the"
			+ " algorithms. The default value is \""
			+ DEFAULT_METRIC.toString().toLowerCase() + "\".";
	...
	private static final String SPEED_DESCRIPTION = "speed and precision of"
			+ " evaluations. The default value is \""
			+ DEFAULT_SPEED.toString().toLowerCase() + "\".";
	...
	/**
	 * Performs checks on the parameters that can't be done when parsing the
	 * values. Indeed, some checks need the value of more than one option to be
	 * performed.
	 */
	private static void check(DataModel dataModel, Long user)
	...
	/**
	 * Generates the parsable string containing the option name and its value.
	 * Adds also a space character at the end.
	 */
	private static String generateLongOption(String optionName,
	...
	private static Options getOptions() {
		...
		return new Options().addOption(dataModel).addOption(user)
				.addOption(algorithms).addOption(metric).addOption(speed)
				.addOption(coverage);
	}
	...
	/**
	 * Parses and checks the given arguments, runs the selection process and
	 * print the result.
	 *
	 * @throws TasteException
	 */
	public static void parse(String[] args) throws ParseException, IOException,
		...
		String dataModelValue = commandLine
				.getOptionValue(DATAMODEL_LONG_OPTION);
		...
		List<RecommenderType> algorithms = parseAlgorithms(commandLine
				.getOptionValues(ALGORITHMS_LONG_OPTION));
		MetricType metric = parseMetric(commandLine
				.getOptionValue(METRIC_LONG_OPTION));
		SpeedOption speed = parseSpeed(commandLine
				.getOptionValue(SPEED_LONG_OPTION));
		double coverage = parseCoverage(commandLine
				.getOptionValue(COVERAGE_LONG_OPTION));

		// the parsing is finished, execute
		check(dataModel, user);
		Optional<RecommenderEvaluation> selection = select(dataModel, user,
				algorithms, metric, speed, coverage);
	 ...
	 */
	private static List<RecommenderType> parseAlgorithms(
			String[] algorithmsValue) {
		...
		if (result.isEmpty()) {
			throw new IllegalArgumentException("Error: at last one algorithm"
					+ " must be provided.");
		}
	...
	private static double parseCoverage(String coverageValue) {
		...
		try {
			...
			throw new IllegalArgumentException("Error: the provided coverage"
					+ " number is not a valid number.", e);
		}

		// check the value
		if (result < 0 || 1 < result) {
			throw new IllegalArgumentException("Error: the minimum coverage"
	 ...
	 */
	private static DataModel parseDataModel(String dataModelValue)
			throws TasteException {
	...
	private static void printSelection(
			Optional<RecommenderEvaluation> optionalSelection,
			String dataModelFilepath, String user) {
		if (!optionalSelection.isPresent()) {
			System.out
					.println("Unable to find a algorithm. The minimum coverage"
							+ " value may be too high.");
		}

		RecommenderEvaluation selection = optionalSelection.get();
		StringBuilder commandLine = new StringBuilder();
		RecommenderType algorithm = selection.getRecommenderConfiguration()
				.getType();

		// algorithm
		commandLine
				.append(generateLongOption(
						RecommendCommandParser.ALGORITHM_LONG_OPTION,
						algorithm.name()));

		// model
		commandLine
				.append(generateLongOption(
						RecommendCommandParser.DATAMODEL_LONG_OPTION,
						dataModelFilepath));

		// user
		commandLine.append(generateLongOption(
				RecommendCommandParser.USER_LONG_OPTION, user));
		...
		case SVD_BASED:
			...
			SvdBasedRecommenderConfiguration svdConfiguration = (SvdBasedRecommenderConfiguration) selection
					.getRecommenderConfiguration();
			commandLine.append(generateLongOption(
					RecommendCommandParser.FEATURES_LONG_OPTION,
					Integer.toString(svdConfiguration.getFeatureNumber())));
			commandLine.append(generateLongOption(
					RecommendCommandParser.ITERATIONS_LONG_OPTION,
		...
		case USER_SIMILARITY_BASED:
			...
			UserBasedRecommenderConfiguration userConfiguration = (UserBasedRecommenderConfiguration) selection
					.getRecommenderConfiguration();
			commandLine.append(generateLongOption(
					RecommendCommandParser.NEIGHBORS_LONG_OPTION,
					Integer.toString(userConfiguration.getNeighborNumber())));
			break;

		default:
			throw new UnsupportedOperationException("Error: unable to handle"
					+ " the selected algorithm. The "
					+ SelectCommandParser.class + " class must be updated.");
	...
	private static Optional<RecommenderEvaluation> select(DataModel dataModel,
			Long user, List<RecommenderType> algorithms, MetricType metric,
			SpeedOption speed, double coverage) throws TasteException {
		BestRecommenderSelector selection = new BestRecommenderSelector(
				dataModel, user, metric, speed, FORCED_EVALUATION_PERCENTAGE);
