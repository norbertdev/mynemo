...
/**
 * This class enables to parse a command line to recommend items to an user.
 */
public class RecommendCommandParser {
	...
	private static final String ALGORITHM_DESCRIPTION = "algorithm used"
			+ " to find the remcommended items";
	public static final String ALGORITHM_LONG_OPTION = "algorithm";

	private static final String COMMAND_SYNTAX = "recommend"
			+ "  --algorithm <algo>  --data-model <file>  --user <id>"
			+ "  [--recommendations <number>]  [--neighbors <number>]"
	...
	private static final String DATAMODEL_DESCRIPTION = "data model used by"
	...
	private static final String FEATURES_DESCRIPTION = "number of features for"
			+ " SVD algorithms";
	...
	private static final String ITERATIONS_DESCRIPTION = "number of iterations"
			+ " for SVD algorithms";
	...
	private static final String NEIGHBORS_DESCRIPTION = "maximum allowed"
			+ " neighbors for item similarity based algorithms";
	...
	private static final String RECOMMENDATIONS_DESCRIPTION = "maximum"
	...
	private static final String USER_DESCRIPTION = "target user of the"
			+ " recommender system";
	public static final String USER_LONG_OPTION = "user";

	/**
	 * Performs various checks on the parameters that can't be done when parsing
	 * the values. Indeed, some checks need the value of more than one option to
	 * be performed.
	 */
	private static void check(RecommenderType algorithm, DataModel dataModel,
			Long user, Optional<Integer> features,
			Optional<Integer> iterations, Optional<Integer> neighbors)
			throws TasteException {

		// user and data model
		try {
			if (dataModel.getItemIDsFromUser(user).isEmpty()) {
				throw new IllegalArgumentException("Error: the user has no"
						+ " preference in the data model.");
			}
		} catch (NoSuchUserException e) {
			throw new IllegalArgumentException(
					"Error: the user is not in the data model.", e);
		}

		// features and algorithm
		if (algorithm.getFamily() == RecommenderFamily.SVD_BASED
				&& !features.isPresent()) {
			throw new IllegalArgumentException("Error: the "
					+ FEATURES_LONG_OPTION + " option must be provided for a"
					+ " SVD based algorithm.");
		}

		// iterations and algorithm
		if (algorithm.getFamily() == RecommenderFamily.SVD_BASED
				&& !iterations.isPresent()) {
			throw new IllegalArgumentException("Error: the "
					+ ITERATIONS_LONG_OPTION + " option must be provided for a"
					+ " SVD based algorithm.");
		}

		// neighbors and algorithm
		if (algorithm.getFamily() == RecommenderFamily.USER_SIMILARITY_BASED
				&& !neighbors.isPresent()) {
			throw new IllegalArgumentException("Error: the "
					+ NEIGHBORS_LONG_OPTION + " option must be provided for"
					+ " an item similarity based algorithm.");
		}
	}

	private static void execute(RecommenderType algorithm, DataModel dataModel,
			Long user, Integer maximumRecommendations,
			Optional<Integer> features, Optional<Integer> iterations,
			Optional<Integer> neighbors) {

		RecommenderBuilder builder;
		switch (algorithm.getFamily()) {
		case BASIC:
			builder = new BasicRecommender(new BasicRecommenderConfiguration(
					algorithm));
			break;

		case ITEM_SIMILARITY_BASED:
			builder = new ItemSimilarityRecommender(
					new ItemBasedRecommenderConfiguration(algorithm));
			break;

		case SVD_BASED:
			builder = new SvdBasedRecommender(
					new SvdBasedRecommenderConfiguration(algorithm,
							features.get(), iterations.get()));
			break;

		case USER_SIMILARITY_BASED:
			builder = new UserSimilarityRecommender(
					new UserBasedRecommenderConfiguration(algorithm,
							neighbors.get(), dataModel, true));
			break;

		default:
			throw new UnsupportedOperationException("Error: unable to handle"
					+ " the given algorithm. The recommend command parser must"
					+ " be updated.");
		}

		try {
			System.out
					.println("IMDb ids of the recommended movies (score/100):");
			for (RecommendedItem recommendation : builder.buildRecommender(
					dataModel).recommend(user, maximumRecommendations)) {
				System.out.println(recommendation.getItemID() + " ("
						+ Math.round(recommendation.getValue()) + ")");
			}
		} catch (TasteException e) {
			throw new IllegalStateException("Error: an unknown error occurs"
					+ " while the recommendation is computed.", e);
		}
	}
	...
	private static Options getOptions() {
		...
		Option recommendation = OptionBuilder
				.create(RECOMMENDATIONS_CHAR_OPTION);
		...
		return new Options().addOption(algorithm).addOption(dataModel)
				.addOption(user).addOption(neighbors).addOption(recommendation)
				.addOption(features).addOption(iterations);
	}
	...
	/**
	 * Parses and checks the given arguments, then runs the recommendation
	 * algorithm.
	 */
	public static void parse(String[] args) throws ParseException, IOException,
		...
		RecommenderType algorithm = parseAlgorithm(commandLine
				.getOptionValue(ALGORITHM_CHAR_OPTION));
		long user = parseUser(commandLine.getOptionValue(USER_CHAR_OPTION));
		Integer recommendations = parseRecommendations(Optional
				.fromNullable(commandLine
						.getOptionValue(RECOMMENDATIONS_CHAR_OPTION)));
		Optional<Integer> features = parseFeatures(commandLine
				.getOptionValue(FEATURES_CHAR_OPTION));
		Optional<Integer> iterations = parseIterations(commandLine
				.getOptionValue(ITERATIONS_CHAR_OPTION));
		Optional<Integer> neighbors = parseNeighbors(commandLine
				.getOptionValue(NEIGHBORS_CHAR_OPTION));
		// loading the data model can be long, thus it is the last parsed option
		DataModel dataModel = parseDataModel(commandLine
				.getOptionValue(DATAMODEL_CHAR_OPTION));

		check(algorithm, dataModel, user, features, iterations, neighbors);

		execute(algorithm, dataModel, user, recommendations, features,
	...
	private static RecommenderType parseAlgorithm(String algorithm) {
		...
		try {
			...
			throw new IllegalArgumentException("Error: unable to find the"
					+ " given algorithm.", e);
		}
	 ...
	 */
	private static DataModel parseDataModel(String dataModel)
			throws TasteException {
		if (!new File(dataModel).exists()) {
			throw new IllegalArgumentException(
					"Error: unable to find the the data model file.");
		}
		...
		try {
			...
			throw new IllegalStateException(
					"Error: unable to load the data model.", e);
		}

		// check
		if (result.getNumUsers() == 0 || result.getNumItems() == 0) {
			throw new IllegalArgumentException("Error: the data model doesn't"
					+ " contain any data.");
		}
	...
	private static Optional<Integer> parseFeatures(String featuresValue) {
		...
		try {
			...
			throw new IllegalArgumentException("Error: the provided feature"
					+ " number is not a valid integer.", e);
		}

		if (result <= 0) {
			throw new IllegalArgumentException("Error: the number of features"
	...
	private static Optional<Integer> parseIterations(String iterationsValue) {
		...
		try {
			...
			throw new IllegalArgumentException("Error: the provided iteration"
					+ " number is not a valid integer.", e);
		}

		// check
		if (result <= 0) {
			throw new IllegalArgumentException("Error: the number of"
	...
	private static Optional<Integer> parseNeighbors(String neighborsValue) {
		...
		try {
			...
			throw new IllegalArgumentException("Error: the provided neighbors"
					+ " number is not a valid integer.", e);
		}

		// check
		if (result <= 0) {
			throw new IllegalArgumentException("Error: the number of neighbors"
					+ " must be greater than 0");
		}
	 ...
	 */
	private static Integer parseRecommendations(
			Optional<String> recommendationsValue) {
		...
		try {
			...
			throw new IllegalArgumentException("Error: the provided maximum"
					+ " recommendation number is not a valid integer.", e);
		}
	...
	private static long parseUser(String user) {
		...
		try {
			...
			throw new IllegalArgumentException("Error: the provided user"
					+ " is not a valid id.", e);
		}
