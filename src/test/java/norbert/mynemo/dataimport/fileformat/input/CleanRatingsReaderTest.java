...
import norbert.mynemo.dataimport.fileformat.input.CleanRatingsReader;
...
public class CleanRatingsReaderTest {
	...
	private static final String PROVIDED_A = CONTENT_PREFIX + EXPECTED_A
			+ CONTENT_SUFFIX;
	private static final String PROVIDED_B = CONTENT_PREFIX + EXPECTED_B
			+ CONTENT_SUFFIX;
	private static final String PROVIDED_C = CONTENT_PREFIX + EXPECTED_C
			+ CONTENT_SUFFIX;
	...
	private String readWithBuffer(BufferedReader bufferReader, char[] buffer,
	...
	private void readWithBufferSize(char[] buffer, int offset, int length) {
		...
		obtainedString = readWithBuffer(new BufferedReader(new StringReader(
				PROVIDED_A)), buffer, offset, length);
		assertEquals(EXPECTED_A, obtainedString);

		obtainedString = readWithBuffer(new BufferedReader(new StringReader(
				PROVIDED_B)), buffer, offset, length);
		assertEquals(EXPECTED_B, obtainedString);

		obtainedString = readWithBuffer(new BufferedReader(new StringReader(
				PROVIDED_C)), buffer, offset, length);
